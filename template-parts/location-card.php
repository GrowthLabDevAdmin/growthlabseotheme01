...
<div class="location-card <?= isset($args['classes']) ? $args['classes'] : '' ?>">
    <div class="location-card__wrapper">
        ...
        <div class="location-card__inner tx-center">
            ...
            $city = $tp_url ? '<a href="' . esc_url($tp_url) . '" target="_blank" aria-label="' . esc_attr($location['city']) . '">' : '';
            $city .= $location['city'];
            $city .= $tp_url ? "</a>" : '';
            ?>
            ...
            <?php if ($location['phone']): ?>
                <a href="tel:+1<?= get_flat_number($location['phone']) ?>" class="location-card__btn btn btn--secondary" aria-label="<?= esc_attr($location['phone']) ?>">
            ...
            <?php if ($location['google_map_url']): ?>
                <a href="<?= $location['google_map_url'] ?>" class="location-card__cta" aria-label="<?= esc_attr( filterContentByLanguage() ? 'Obtener Direcciones' : 'Get Directions' ) ?>">
